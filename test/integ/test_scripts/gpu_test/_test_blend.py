...
def main(job_history_dir: str, output_dir_in_scene: str, output_dir_in_submitter: str):
    ...
    try:
        ...
        # Find first available GPU device type and enable it
        ...
        if non_cpu_device:
            detected_device_type = non_cpu_device.type
            for device in devices:
                device.use = False
            non_cpu_device.use = True
            print(
                f"Using device type: {detected_device_type}, enabled device: {non_cpu_device.name}"
            )

        # Write detected device type to file for test validation
        with open(Path(job_history_dir) / "detected_gpu_type.txt", "w") as f:
            f.write(detected_device_type or "NONE")
